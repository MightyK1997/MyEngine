...
namespace
{
	...
	// that is used by the engine to store mesh data!
...
MStatus eae6320::cMayaMeshExporter::writer( const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode )
{
	...
	std::vector<sMaterialInfo> materialInfo;
	{
		...
		status = FillVertexAndIndexArrays( uniqueVertices, shadingGroups, triangles, vertexArray, indexArray, materialInfo );
		if ( !status )
		{
			return status;
		}
...
namespace
{
		...
		std::vector<MObject>& o_shadingGroups )
	{
		...
		for ( MItDag i( MItDag::kDepthFirst, MFn::kMesh ); !i.isDone(); i.next() )
		{
			MDagPath dagPath;
			i.getPath( dagPath );
			const auto status = ProcessSingleDagNode( dagPath, o_uniqueVertices, o_triangles, o_shadingGroups, map_shadingGroupNamesToIndices );
			if ( !status )
			{
				return status;
			}
		...
		std::vector<MObject>& o_shadingGroups )
	{
		...
		const auto status = MGlobal::getActiveSelectionList( selectionList );
		if ( status )
		{
			std::map<std::string, size_t> map_shadingGroupNamesToIndices;
...
//			fout << std::fixed;
				...
				"{" "\n";
			{
				...
				for (int i = 0; i < i_vertexArray.size(); i++)
				{
					...
					fout << "UV=""{" << i_vertexArray[i].vertex.u << "," << i_vertexArray[i].vertex.v << "}"",""\n";
					fout << "}" "," "\n";
				}
				fout << "}," "\n";
				fout<< "IndexBuffer = " "\n" "{" "\n";
