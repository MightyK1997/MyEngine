	...
	namespace Renderer
	{
		...
		bool GetDimensions(Texture * i_pTexture, unsigned int & o_Width, unsigned int & o_Height, unsigned int & o_Depth)
		{
			...
			case D3D11_RESOURCE_DIMENSION_TEXTURE2D:
			{
				...
				HRESULT hResult = i_pTexture->m_pTextureResource->QueryInterface(__uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&pTexture));
				assert(hResult == S_OK);
				assert(pTexture);
			...
			case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
			{
				...
				HRESULT hResult = i_pTexture->m_pTextureResource->QueryInterface(__uuidof(ID3D11Texture3D), reinterpret_cast<void**>(&pTexture));
				assert(hResult == S_OK);
				assert(pTexture);
