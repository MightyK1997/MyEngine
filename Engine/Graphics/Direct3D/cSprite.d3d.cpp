...
namespace eae6320
{
	namespace Graphics
	{
		cResult cSprite::Initialize(eae6320::Graphics::VertexFormats::sSprite* i_inputSprite, unsigned i_VertexCount)
		{
			...
			// Initialize vertex format
			{
				...
				if (result = eae6320::Platform::LoadBinaryFile("data/Shaders/Vertex/spriteVertexInputLayout.shader", vertexShaderDataFromFile, &errorMessage))
				{
				...
				// Vertex Buffer
				{
					...
					{
						const auto bufferSize = i_VertexCount * sizeof(eae6320::Graphics::VertexFormats::sSprite);
					...
					{
						initialData.pSysMem = i_inputSprite;
				...
				// Specify what kind of data the vertex buffer holds
				{
					...
					direct3dImmediateContext->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
				}
			OnExit:
				return result;
			}
		...
		void eae6320::Graphics::cSprite::Draw(bool bindVertex)
		{
				auto* const direct3dImmediateContext = sContext::g_context.direct3dImmediateContext;
				{
					if (bindVertex)
