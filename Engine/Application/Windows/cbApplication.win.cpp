...
//			return TRUE;
	...
	{
		UINT size;
		GetRawInputData((HRAWINPUT)i_lParam, RID_INPUT, NULL, &size, sizeof(RAWINPUTHEADER));
		...
		if (GetRawInputData((HRAWINPUT)i_lParam, RID_INPUT, byte, &size, sizeof(RAWINPUTHEADER)) == size)
			eae6320::UserInput::SetRawInputData(byte);
	}
	break;
	}
