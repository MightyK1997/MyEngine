	...
	)
{
	...
	const float materialConstant = 15;

    const float4 materialReflectance = {255,215,0,1};
	...
	const float3 reflectedCameraVector = reflect(-viewDist, i_normals);

	const float4 sampleFromEnvMap = SampleTextureCube(g_EnvMap, g_samplerState, reflectedCameraVector);

    const float3 normalizeV = normalize(viewDist);
    const float3 H = normalize(normalizeL+normalizeV);

	const float4 reflectionValue = (materialConstant + ((1-materialConstant) * (1- pow((dot(i_normals, reflectedCameraVector)), 5))) ) * materialReflectance;
	...
	//New Specular
	//[D(h) * F(l,h)] / 8
	const float4 specularLight = saturate(blinnPhong * fresnelEquation * 0.125 * (finalSmoothness + 2)) * materialReflectance;
	...
	const float4 diffuseOutput = float4(0,0,0,1) + reflectionValue;
	...
	float4 diffusePoint = float4(0,0,0,1) + reflectionValue;

	float4 pointLightOutput = (diffusePoint + specularPoint + ambientLight) * g_PointLightColor * saturate(dot(normalizedPointLightDir, normalizedNormal.xyz)) * 2;

	//o_color = (directionalLightOutput + pointLightOutput) * textureColor * i_color;

	o_color = directionalLightOutput * sampleFromEnvMap;
}
